...
def main():
    ...
    alpha     = parameters["alpha"]    # active stress
    ...
    rho_iso   = parameters["rhoisoend"] /rho_in   # jamming density for isotropic
    ...
    system.create_term("Hxx", [("rho", None)], [pii, 0, 0, 0])                                #external field term
    ...
    # Define vy
    system.create_term("vy", [('vRHSy', None), ('rho', (np.power, -1))], [+1, 0, 0, 0])
    # Define kappa_a_xy
    ...
    nclusters = ncluster * np.int32(rho_seed/(50/rho_in)) #there's about nclusters when plated at 50 cells/mm^2, scale appropriately
    ...
    for t in tqdm(range(n_steps)):
        system.get_field('noiseQxx').set_real(KQ*rhoJrho.get_real()*np.heaviside(rhoJrho.get_real(), 0)*np.random.normal(size=[mx, my]))
        system.get_field('noiseQxx').synchronize_momentum()
        
        system.get_field('noiseQxy').set_real(KQ*rhoJrho.get_real()*np.heaviside(rhoJrho.get_real(), 0)*np.random.normal(size=[mx, my]))
...
def set_rho_Q_islands(rhofield, Qxxfield, Qxyfield, ncluster, rcluster, rhocluster, rhobg, rhoseed, rho_max, grid_size):
    ...
    Qxx = Sinit*(np.cos(2*itheta))/2
    Qxy = Sinit*(np.sin(2*itheta))/2

    #Qxx = gaussian_filter(Qxx, 1, mode='wrap') #kernel size = radii
    #Qxy = gaussian_filter(Qxx, 1, mode='wrap') #kernel size = radii

    Qxxfield.set_real(Qxx)
    Qxxfield.synchronize_momentum()
    Qxyfield.set_real(Qxy)
