...
if (( $# != 7 )); then
    echo "Usage: run_model.s model_name epsilon alpha chi lambda rhoseed run"
    exit 1
fi


model=$1
epsilon=$(python3 -c "print('{:.2f}'.format($2))")
...
rhonemend=700
rrhoend=1.5
ncluster=16 #10 for mx=my=100 and rhoseed =0.5. scale with system size. 
rcluster=3.2
rhocluster=180 #1.8
rhobg=15
mx=100
...
save_dir="${sh_dir}/data/cluster/$model/ncluster_${ncluster}_rhocluster_${rhocluster}_rhobg_${rhobg}/invgamma0_${invgamma0}_rhoseed_${rhoseed}_KQ_${KQ}/epsilon_${epsilon}_alpha_${alpha}_chi_${chi}_lambda_${lambda}/run_${run}"
